...
export default {
  ...
  methods: {
    _initScroll () {
      ...
      // 这里的bscroll 是直接读取的数据 而不是派发dispatch获取的 因此不需要回调函数(前面有用到过)
      const ul = this.$refs.picsUl
      const liWidth = 120
      const space = 6
      const count = this.info.pics.length
      ul.style.width = (liWidth + space) * count - space + 'px'
      this.imgScroll = new BScroll('.pic-wrapper', {
        scrollX: true
      })
    }
  },
  // 当前页面刷新： 先显示初始化 有新来的数据再去更新
  watch: {
    info () {
      this.$nextTick(() => {
        this._initScroll()
  ...
  // 在当前路由刷新会出现xxx没有数据的情况 因为数据都是在第一个路由商品那儿获取完的
  // 因此刷新的时候 在初始化的时候是不能再第一时间获取到数据的
}
</script>
